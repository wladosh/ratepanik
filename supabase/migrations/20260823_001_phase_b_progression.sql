...
INSERT INTO achievements (id, title, description, icon_key, trigger) VALUES
  ('first_game',      'Erster Panik-Anfall',    'Du hast dein erstes Spiel überlebt.',                                         'rp_badge_first_win',   'games_finished >= 1'),
  ('first_win',       'Erster Sieg',            'Jemand hat verloren. Du nicht.',                                               'rp_badge_first_win',   'wins >= 1'),
  ('first_room',      'Gastgeber',              'Du hast einen Raum erstellt.',                                                 'rp_badge_party_host',  'rooms_created >= 1'),
  ('exact_hit',       'Bullseye',               'Exakter Treffer bei einer Schätzfrage.',                                       'rp_badge_streak_3',    'number_guess_exact_hits >= 1'),
  ('exact_streak_3',  'Hellseher',              '3 exakte Schätz-Treffer in einer Session.',                                    'rp_badge_streak_3',    'number_guess_exact_hits_in_session >= 3'),
  ('close_call',      'Knapp daneben ist auch… nah', 'Innerhalb von 5% der richtigen Zahl (ohne exakt).',                       'rp_badge_streak_3',    'number_guess_within_5pct >= 1'),
  ('wild_guess',      'Mutig',                  'Deine Schätzung war um mehr als das 10-Fache daneben.',                        'rp_badge_streak_3',    'number_guess_off_by_10x >= 1'),
  ('perfect_pick',    'Saubere Weste',          'Eine Pick-Correct-Runde mit 0 Fehlern (alle 4 richtig, keine Falschen).',      'rp_badge_streak_3',    'pick_correct_perfect_rounds >= 1'),
  ('pick_streak_3',   'Kartenscharf',           '3 perfekte Pick-Correct-Runden in einer Session.',                             'rp_badge_streak_3',    'pick_correct_perfect_in_session >= 3'),
  ('almost',          'Fast',                   'Genau 3 von 4 richtigen bei Pick-Correct.',                                    'rp_badge_streak_3',    'pick_correct_exactly_3 >= 1'),
  ('panic_pick',      'Alles falsch',           '0 Richtige in einer Pick-Correct-Runde.',                                      'rp_badge_streak_3',    'pick_correct_zero_correct >= 1'),
  ('clutch',          'Clutch',                 'Von hinten auf Platz 1 in einer Runde/Session.',                                'rp_badge_first_win',   'came_from_behind_win >= 1'),
  ('rematch',         'Nochmal!',               'Direkt nach einem Spiel ein Rematch gestartet.',                                'rp_badge_party_host',  'rematch_started >= 1'),
  ('full_lobby',      'Volle Hütte',            'Spiel mit maximaler Spieleranzahl gestartet.',                                  'rp_badge_party_host',  'players_at_start == max_players'),
  ('night_owl',       'Nachtschicht',           'Spiel zwischen 00:00 und 05:00 (User-Lokalzeit) beendet.',                     'rp_badge_streak_3',    'finished_hour in 0..4'),
  ('games_10',        'Stammgast',              '10 Spiele beendet.',                                                            'rp_badge_first_win',   'games_finished >= 10'),
  ('wins_5',          'Gewohnheitstäter',       '5 Siege.',                                                                      'rp_badge_first_win',   'wins >= 5'),
  ('exact_10',        'Zahlenflüsterer',        '10 exakte Schätz-Treffer insgesamt.',                                           'rp_badge_streak_3',    'number_guess_exact_hits >= 10'),
  ('perfect_10',      'Pick-Profi',             '10 perfekte Pick-Correct-Runden insgesamt.',                                    'rp_badge_streak_3',    'pick_correct_perfect_rounds >= 10')
