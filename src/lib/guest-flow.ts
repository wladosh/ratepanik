 ...
 * different room. Same-room `/?join=CODE` (tab return / iOS app-switch)
 * keeps the seat so we do not wipe playerId and force a second join.
 */
...
/**
 * Guests with an active session can restore on bare `/`.
 * iOS often drops `?join=` after an app-switch; destroying the session
 * would send them through GuestExitToLogin.
 */
export function shouldRestoreMatchOnBareHome(_isGuest?: boolean): boolean {
  return true;
}
